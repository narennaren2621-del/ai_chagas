...
import 'package:chagas_predictor/services/picker/csv_file_picker.dart';
...
import 'package:chagas_predictor/services/ml/fastapi_prediction_service.dart';
...
import 'package:chagas_predictor/widgets/ecg/ecg_digitizer_view.dart';
...
class _PredictionPageState extends State<PredictionPage> {
  ...
  Future<void> _processEcgImage({required XFile xFile}) async {
    setState(() {
      _isUploading = true;
      _uploadError = null;
    });
    ...
    try {
      ...
      setState(() {
        ...
        _uploadError = e.toString().replaceFirst('FormatException: ', '');
        _isUploading = false;
      });
    }
  }
  ...
  Future<void> _runLstmPrediction() async {
    ...
    final prediction = await FastApiPredictionService.predictRecord(record);

    // Automatically save user-input risk assessment into History
    final sourceName = _inputMode == 0
        ? (_digitizedEcgResult?.imageName ?? 'ECG Photo Scan')
        : (_preprocessingResult?.fileName ?? 'Uploaded CSV File');

    AssessmentHistoryService().saveAssessmentRecord(
      patientDetails: PatientService().patientDetails,
      inputMode: _inputMode == 0 ? 'ECG Image Scan' : 'CSV Dataset',
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: const Icon(Icons.history_rounded, color: Color(0xFF006B67)),
          ),
          if (_currentStep > 0)
            TextButton.icon(
  ...
  Widget _buildStep2Preprocessing() {
    ...
    return Column(
      ...
      children: [
        ...
        else if (_inputMode == 1 && _preprocessingResult != null)
          _buildCsvRecordPreviewCard(),
  ...
  Widget _buildCsvRecordPreviewCard() {
    final result = _preprocessingResult!;
    if (result.records.isEmpty) return const SizedBox.shrink();

    final selectedRecord = result.records[_selectedRecordIndex.clamp(0, result.records.length - 1)];
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      const Text(
                        'CSV Record Preprocessed Parameters',
                        style: TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.w800,
                      ...
                      Text(
                        'File: ${result.fileName} • Record ID: ${selectedRecord.examId}',
                        style: const TextStyle(
                          fontSize: 12,
              ...
              if (result.records.length > 1)
                DropdownButton<int>(
                  ...
                  items: List.generate(result.records.length, (i) {
                    final rec = result.records[i];
                    return DropdownMenuItem(
                      value: i,
                      child: Text('Record #${i + 1} (${rec.examId})'),
                    );
                  }),
                ),
            ],
          ...
          Wrap(
            ...
            runSpacing: 10,
            children: [
              _buildMiniParamBadge('P-Wave', '${selectedRecord.pWaveDurationMean.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('PR Interval', '${selectedRecord.prIntervalMean.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('QRS Duration', '${selectedRecord.qrsDurationMean.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('QT Interval', '${selectedRecord.qtIntervalMean.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('HRV SDNN', '${selectedRecord.hrvSDNN.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('HRV RMSSD', '${selectedRecord.hrvRMSSD.toStringAsFixed(1)} ms'),
              _buildMiniParamBadge('Patient Age', '${selectedRecord.age.toInt()} yrs'),
              _buildMiniParamBadge('Gender', selectedRecord.isMale ? 'Male' : 'Female'),
            ],
          ),
        ],
      ),
  ...
  Widget _buildMiniParamBadge(String label, String value) {
    return Container(
